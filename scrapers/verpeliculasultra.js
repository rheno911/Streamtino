// ============================================================
//  scrapers/verpeliculasultra.js - Scraper para verpeliculasultra.com
// ============================================================
const cheerio = require("cheerio");
const { get } = require("../utils/http");
const { extractVideoUrl, extractIframes } = require("../utils/extractor");

const BASE = "https://verpeliculasultra.com";
const SOURCE = "VerPeliculasUltra";
...
async function search(query) {
  try {
    ...
    const results = [];
    $("article, .result-item, .pelicula").each((_, el) => {
...
async function getStreams(imdbId, type, season, episode) {
  try {
    ...
    if (type === "series" && season && episode) {
      ...
      const epLink = p$(
        `a[href*="temporada${season}"], a[href*="episodio${episode}"], .episodio-${episode} a`
      ).first().attr("href");
    ...
    for (const iframe of iframes.slice(0, 5)) {
      ...
      if (videoUrl) {
        streams.push({ name: SOURCE, title: `🍿 ${SOURCE}`, url: videoUrl });
      }
    }
    return streams;
  } catch (err) {
...
async function getCatalog(type, skip = 0) {
  try {
    ...
    const section = type === "movie" ? "peliculas" : "series-online";
    const res = await get(`${BASE}/${section}/page/${page}/`, { headers: { Referer: BASE } });
    const $ = cheerio.load(res.data);
    const items = [];
    $("article, .item-pelicula, .movie-item").each((_, el) => {
      const title = $(el).find("h2, h3, .title").first().text().trim();
      const link = $(el).find("a").first().attr("href") || "";
      const img = $(el).find("img").attr("src") || $(el).find("img").attr("data-src") || "";
      if (title && link) {
        items.push({ id: `latam:ultra:${encodeURIComponent(link)}`, type, name: title, poster: img, source: SOURCE, sourceUrl: link });
      }
    });
    return items;
  } catch (err) {
