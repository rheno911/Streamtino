// ============================================================
//  scrapers/estrenosanime.js - Scraper para estrenosanime.net
// ============================================================
const cheerio = require("cheerio");
const { get } = require("../utils/http");
const { extractVideoUrl, extractIframes } = require("../utils/extractor");

const BASE = "https://estrenosanime.net";
const SOURCE = "EstrenosAnime";
...
async function getStreams(imdbId, type, season, episode) {
  try {
    ...
    if (episode) {
      ...
      const epLink = p$(`a[href*="episodio-${episode}"], .cap-${episode} a`).first().attr("href");
...
async function getCatalog(type, skip = 0) {
  try {
    ...
    $("article, .anime-item").each((_, el) => {
      ...
      if (title && link) {
        items.push({ id: `latam:estrenosanime:${encodeURIComponent(link)}`, type: "series", name: title, poster: img, source: SOURCE, sourceUrl: link });
      }
    });
    return items;
  } catch (err) {
