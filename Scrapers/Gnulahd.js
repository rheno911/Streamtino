// ============================================================
//  scrapers/gnulahd.js - Scraper para ww3.gnulahd.nu
// ============================================================
const cheerio = require("cheerio");
const { get } = require("../utils/http");
const { extractVideoUrl, extractIframes } = require("../utils/extractor");

const BASE = "https://ww3.gnulahd.nu";
const SOURCE = "GnulaHD";
...
async function search(query) {
  try {
    ...
    const results = [];
    $("article, .result-item, .TPost").each((_, el) => {
...
async function getStreams(imdbId, type, season, episode) {
  try {
    ...
    const $ = cheerio.load(res.data);
    const link = $("article a, .TPost a, h2 a").first().attr("href");
    if (!link) return [];
    ...
    if (type === "series" && season && episode) {
      ...
      // GnulaHD usa estructura Dooplay
      const seasonBtn = p$(`.aa-tbs-btn[data-tab="tab-season-${season}"]`);
      const epLink = p$(
        `#tab-season-${season} a:nth-child(${episode}), 
         .episodio-${episode} a,
    ...
    for (const iframe of iframes.slice(0, 5)) {
      ...
      if (videoUrl) {
        streams.push({ name: SOURCE, title: `📺 ${SOURCE} HD`, url: videoUrl });
      }
    }
    return streams;
  } catch (err) {
...
async function getCatalog(type, skip = 0) {
  try {
    ...
    const $ = cheerio.load(res.data);
    const items = [];
    $("article, .TPost, .movie-item").each((_, el) => {
      const title = $(el).find("h2, h3, .Title").first().text().trim();
      const link = $(el).find("a").first().attr("href") || "";
      const img = $(el).find("img").attr("src") || $(el).find("img").attr("data-src") || "";
      const year = $(el).find(".Year, .year").text().match(/\d{4}/)?.[0] || "";
      if (title && link) {
        items.push({
          id: `latam:gnulahd:${encodeURIComponent(link)}`,
          type,
          name: title,
          poster: img,
          year,
