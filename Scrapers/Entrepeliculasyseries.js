// ============================================================
//  scrapers/entrepeliculasyseries.js - Scraper para entrepelicilasyseries.nz
// ============================================================
const cheerio = require("cheerio");
const { get } = require("../utils/http");
const { extractVideoUrl, extractIframes } = require("../utils/extractor");

const BASE = "https://entrepelicilasyseries.nz";
const SOURCE = "EntrePeliculasySeries";
...
async function search(query) {
  try {
    ...
    const results = [];
    $("article, .result-item, .movie-item").each((_, el) => {
    ...
    if (type === "series" && season && episode) {
      ...
      const epLink = p$(
        `a[href*="temporada-${season}"][href*="episodio-${episode}"],
         a[href*="${season}x${String(episode).padStart(2, "0")}"]`
      ).first().attr("href");
...
async function getCatalog(type, skip = 0) {
  try {
    ...
    $("article, .movie-item").each((_, el) => {
      ...
      if (title && link) {
        items.push({ id: `latam:entrepeli:${encodeURIComponent(link)}`, type, name: title, poster: img, source: SOURCE, sourceUrl: link });
      }
    });
    return items;
  } catch (err) {
