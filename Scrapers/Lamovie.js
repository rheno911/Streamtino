// ============================================================
//  scrapers/lamovie.js - Scraper para la.movie
// ============================================================
...
async function search(query) {
  try {
    ...
    $("article a, .result a, .movie-item a").each((_, el) => {
      ...
      if (title && href) results.push({ title, link: href, img });
    });
    return results;
  } catch (err) {
...
async function getCatalog(type, skip = 0) {
  try {
    ...
    $("article, .movie-item").each((_, el) => {
      ...
      const img = $(el).find("img").attr("src") || "";
      if (title && link) {
        items.push({ id: `latam:lamovie:${encodeURIComponent(link)}`, type, name: title, poster: img, source: SOURCE, sourceUrl: link });
      }
    });
    return items;
  } catch (err) {
